...
### LSTM による固有表現認識
LSTMを使うことで、テキストから、各単語に対して固有表現認識を行うモデルを構築できる

### 双方向LSTM(Bi-LSTM)
過去の情報だけではなく、未来の情報も予測に使うモデル。
自然言語処理では、文全体で意味を成すので、各単語の意味を捉えるのに将来（文の後に出てくる単語）の情報は有用になる。

### BERT
ここまでの手法は、文脈を意識することはできない。
文脈も用いることができる手法の中でもメジャーな方法として、BERTが挙げられる。
BEATの特徴に「事前学習」と「Fine-tuning」がある。

#### 事前学習
２つのタスクを事前に行うことがポイントになる。
- マスクされた単語の予測
    - 文中の一定割合を [MASK] というマスクに置き換えて、そこに何が入るかを予測するタスク
    - これによって双方向の文脈を意識した学習を行うことになる
- 与えた２つの文が隣接文(連続する文)かどうかの予測
    - 2つの文を与えて、それが連続する分かどうかを学習
    - これによって、文の間の関係性（文脈）も意識できる
    - 質疑応答のような入力(質問文)と出力(回答が含まれる文)の関係が重要なタスクに対しても対応できるような学習を行う

#### Fine-tuning
事前学習において、大量のデータによって汎用的な特徴は獲得している。それに対して、少量のデータで自分の解きたいタスクに対する性能を上げる。
## 11章 系列変換
[実践例](https://colab.research.google.com/drive/1e7c1bwlNWtJ9TQjjOH3SafTH4x1iyXVi?usp=sharing)
### 系列変換とは
入力の系列を別の系列に変換して出力するモデル。自然言語処理で言えば、機械翻訳、対話システム、テキスト要約などがわかりやすい例。
自然言語処理以外でも、音声認識やイメージキャプションなど。

### 系列変換の仕組み
過去に、Google翻訳は統計ベースの手法を使っており、コードは５０万行にも達していた。
近年では、系列変換モデルを使ったニューラル機械翻訳のシステムはコードとしては５００行程度になっている。

大枠の仕組みとしては、入力側、出力側でそれぞれ異なる系列のデータを用いるため、2つのRNNを組み合わせたモデルが多い。

#### エンコーダ
入力の系列を固定朝のベクトルに変換するRNN。

#### デコーダ
エンコーダから受け取ったベクトルと、デコーダの一つ前の出力を使って、次に出力を生成するRNN。

### BELU(機械翻訳の評価)
プロの翻訳者の翻訳をお手本にして、それに似た出力を作れるほど良い、とする評価方法。

### アテンション
エンコーダの問題点として、入力が大きくなった場合、最初の方の情報が失われてしまうことが挙げられる。
その解決のために、アテンションでは、エンコーダの最終状態だけではなく、各ステップ（途中の状態）に重み付けして加重平均としたもの（文脈ベクトル）を使用する。
## 12章 機械学習とクラウド
### AutoML
機械学習に必要な各操作、データの準備、モデル構築と学習、モデルのデプロイといった作業を自動化する
### モデル選択
様々なモデルがあるが、タスクによって適するものは異なり、唯一解は存在しない。
[scikit-learn のチートシート](https://scikit-learn.org/stable/tutorial/machine_learning_map/index.html)などは参考にできるが、それが必ずしもいいとも限らない。
### ニューラルアーキテクチャサーチ（NAS)
AIを構築できるAI、とも呼ばれる。NNの構造設計を自動化する技術。計算量の多さが課題。
Cloud AutoML などがサービスとして存在。コストの高さと、モデルのエクスポートができない点が欠点。
# Links
