...
### 文章分類に使用するBERT
- モデルは `cl-tohoku/bert-base-japanese-whole-word-masking`
- クラスは `BertForSequenceClassification`
### 文章分類とは
...
### Multi-hot ベクトル
- one-hot ベクトルのように、0 と 1 で構成される
- 複数の要素、当てはまるものが 1 となるベクトル

### マルチラベル文章分類に使用するBERT
- モデルは `cl-tohoku/bert-base-japanese-whole-word-masking`
- クラスは `BertForSequenceClassificationMultiLabel`

### chABASA-dataset
- TIS 公開の、有報を用いて作成されたマルチラベルのベガポジ判定のデータセット
    - 文章中の単語に「ネガティブ」「ポジティブ」「ニュートラル」のラベルを付与
- これによって、文章中にどんな単語が含まれるかで、マルチラベルをつける
    - ネガティブかつポジティブな文章、などもある

## 8章 固有表現抽出
### 固有表現抽出
- 文章から、人名、組織名といった固有名詞を抽出するタスク
    - 固有表現のカテゴリーは、人名や組織名だけではなく様々
    - カテゴリには IREX, 拡張固有表現階層、などが用いられる

### 文章表現の正規化
- 文章表記の揺れ（全角・半角）などがモデルの精度に影響を及ぼすことがある
- 正規化として、 `unicodedata.normalize(target)` 関数などが用いられる

### IO法
- 固有表現を I(Inside), それ以外をO(Outside) でラベリングして分類
- I に連続するものは固有表現の一部として分類
- 同じタイプの固有表現が連続したとき、それらをひとまとめに連結したものが固有表現として認識されてしまう
    - (例)`日米の代表が...` -> `日`: 固有表現(国), `米`: 固有表現(国) -> `日米`: 固有表現(国) など

### 固有表現が正しく抜き出せないとき
- `昨日のみらい事務所との打ち合わせ` -> `昨日, のみ, らい, 事務所, と, の, ...` のように、固有表現が途中で分割されてしまう場合など
    - 先に固有表現で文章を分割しておく、などが有効
    - `昨日のみらい事務所との打ち合わせ` -> `昨日の` `みらい事務所` `との打ち合わせ` など

### 固有表現抽出に使用するBERT
- モデルは `cl-tohoku/bert-base-japanese-whole-word-masking`
- クラスは `BertForTokenClassification`

### BIO法
- IO法の、連続する固有表現が連結されて認識されてしまう問題点を改善
- 固有表現の先頭をB(Beginning), 先頭以外をI(Inside), それ以外をO(Outside) でラベリングして分類
...
### 文章校正に使用するBERT
- モデルは `cl-tohoku/bert-base-japanese-whole-word-masking`
- クラスは `BertForMaskedLM`

### BERTでうまくいかないパターン
- 単語の区切りの数が変わってしまうようなパターンは BERT でうまく処理するのが難しい
    - 当初は実行を想定：当初、は、実行、を、想定
    - 投書は実行を想定：投、##書、は、実行、を、想定
- 今回は、上記のようなパターンは含まない

### 処理のための関数
以下の関数を準備する
1. （学習用）誤変換した文章と正しい文章が与えられたときに、それぞれの文章を符号化し、モデル入力可能な形式に変換する
2. （推論用）誤変換した文章を符号化するとともに、各トークンの文章中での位置を特定する関数
3. （推論用）BERTが出力したトークン列が与えられたときに、空白を考慮して、それを文章として出力する関数

## 10章 文章ベクトルを用いたデータの可視化と類似文章検索
### 文章ベクトル
- 文章をベクトルとして特徴づけたもの
- 何らかの形で、文章の意味を表現
- コサイン類似度の形で、文章同士の類似度を表現することもできる

### BERT による文章ベクトルの生成
- BERT によって、文章中のトークンに対してベクトルを出力できる
- 全てのトークンのベクトルを集約し、単一のベクトルにすることで、その文章をベクトル化できる

#### 文章ベクトル生成の方法
大きく分けて2つの方法が存在
- トークン列の先頭の特殊トークン`[CLS]`に対応するベクトルを用いる
- 全ての単語ベクトルの平均を用いる
最近のトレンドは後者。

###　文章ベクトルの可視化
文章ベクトルは高次元のベクトルなので、そのままだと理解するのが難しい。

そこで、可視化のためには次元圧縮が用いられる。
